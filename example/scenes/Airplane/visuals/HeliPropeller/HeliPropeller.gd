...
func _on_Engine_update_interface(values):
    target_rotation_speed = 0.0 if not values["engine_active"] else 0.2 + \
        values["engine_power"] * 0.8
...
func _physics_process(delta):
    if is_changing_speed:
        ...
        if abs(target_rotation_speed - rotation_speed) <= frame_move_delta:
            ...
            rotation_speed += (
                frame_move_delta) if target_rotation_speed > rotation_speed else (-frame_move_delta)
