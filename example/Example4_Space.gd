...
func _physics_process(delta):
    if is_instance_valid(aircraft):
        ...
        if is_reloading_fuel:
            ...
            is_aircraft_full = aircraft.load_energy(
                "fuel", amount_per_second * delta)
            if is_aircraft_full:
                is_reloading_fuel = false
...
func air_calculate_density_temperature():
    # Changing air temperature and density is game responsability, not
    # aircraft's
    ...
    if is_on_planet:
        ...
        aircraft.AirTemperature = lerp(
            25.0, -aircraft.ZERO_C_IN_K, altitude_normalized_to_atmosphere)
...
func switch_to_planet(planet=null):
    ...
    if not planet:
        ...
        is_on_planet = true
        planet_influence_radius = planet.get_node(
            "InfluenceDistance").shape.radius
