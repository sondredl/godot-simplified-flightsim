...
# Deprecated, use node's own position instead
@export var EnginePosition: Vector3 = Vector3.ZERO
...
func engine_start():
    ...
    sfx_tween.tween_property(
        sfx_engine_loop,
        "pitch_scale",
        power_to_pitch(current_power),
        1.0)
...
func engine_set_power(value: float):
    ...
    sfx_tween.tween_property(
        ...
        0.3).set_trans(
        Tween.TRANS_LINEAR)
    sfx_tween.parallel().tween_property(
        sfx_engine_loop,
        "pitch_scale",
        power_to_pitch(current_power),
        0.3).set_trans(
