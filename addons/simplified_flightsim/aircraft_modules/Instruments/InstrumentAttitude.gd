...
func process_physic_frame(delta):
    ...
    match aircraft.WorldType:
        aircraft.WorldTypes.LINEAR:
            local_north = aircraft.to_local(
                aircraft.global_transform.origin -
                aircraft.world_ref.global_transform.basis.z).normalized()
        ...
        aircraft.WorldTypes.SPHERICAL:
            ...
            # Magnetic vector is a composition of world's basis.y and the
            # equator projection to center
            var equator_proj_center = aircraft.world_ref.to_global(Vector3(
                -aircraft.direction_in_world_frame.x,
                0.0,
                -aircraft.direction_in_world_frame.z)
            ) - aircraft.world_ref.global_transform.origin
            # lat_normalized range: [-1, +1] (pole-to-pole)
            var lat_normalized = current_latitude / (0.5 * PI)
            ...
            local_north = aircraft.to_local(
                aircraft.global_transform.origin + mag_vector)

    current_roll = atan2(-local_countergravity.x, local_countergravity.y)
    current_pitch = atan2(-local_countergravity.z,
                          Vector2(
                              local_countergravity.x,
                              local_countergravity.y).length()  # Projection on XY
                          )
