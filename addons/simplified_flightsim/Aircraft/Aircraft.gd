...
const STEFAN_BOLTZMANN = 5.670374419 * 0.00000001
...
# Spherical world is real life: away from reference origin is UP,
# reference Y axis is magnetic NORTH
enum WorldTypes {
    LINEAR,
    SPHERICAL
}
...
func _ready():
    ...
    internal_world_reference = get_node_or_null(
        "/root/WorldOrientationReference")  # avoids creating more than one
...
func find_modules_by_type_and_tag(module_type: String, module_tag: String) -> Array:
    var result = []
    for module in modules:
        if (module.ModuleType == module_type) and (
                module_tag in module.ModuleTags):
            result.append(module)
    return result

func register_safe_collider(collider: CollisionShape3D):
    if collider not in safe_colliders:
...
func prepare_physics_variables():
    ...
    match WorldType:
        WorldTypes.LINEAR:
            local_gravity_direction = to_local(
                global_transform.origin -
                world_ref.global_transform.basis.y).normalized()
            if AltitudeEnabled:
        ...
        WorldTypes.SPHERICAL:
            local_gravity_direction = to_local(
                world_ref.global_transform.origin).normalized()
            if AltitudeEnabled:
    ...
    if EnableTemperatureCalculations:
        stagnation_temperature_K = air_temperature_K * \
            (1.0 + (RATIO_OF_SPECIFIC_HEATS - 1.0) * 0.5 * (mach_speed * mach_speed))

    var energy_levels = {}
    for energy_cont in energy_containers:
        if energy_cont.ContainerActive:
            if energy_cont.EnergyType not in energy_levels:
    ...
    # Any external nodes should use this callback to make sure variables are
    ...
    lift_intensity = LiftFactor * AirDensity * \
        (AIR_DENSITY_RHO * forward_air_speed_squared) / 2  # (rho * V^2)/2
...
func process_physics_frame(delta):
    ...
    # right wing of plane in global frame
    ...
    angular_damp = 1.0 + \
        ((drag_intensity_vector.length_squared()) * 0.01) * AirDensity
    ...
    if EnableTemperatureCalculations:
        ...
        # https://en.wikipedia.org/wiki/Nusselt_number -> average number for
        # forced laminar flow: 0.664
        ...
        # therefore the equation input and output is both temperature (per unit
        # of time)
        var delta_to_stagnation = (stagnation_temperature_K - local_temperature_K)
        # min() avoids going beyond stagnation
        var temperature_intake = min(DragHeatRate * heat_velocity_factor * delta_to_stagnation * delta, delta_to_stagnation)
        ...
        local_temperature = max(local_temperature +
                                (temperature_intake -
                                 radiated_temperature_outtake), -
                                ZERO_C_IN_K)  # max() avoids going below 0K
    ...
    # Positive means to the aircraft roof
    var vertical_acceleration_normalized = (up_vector.dot(linear_acceleration) / EARTH_GRAVITY) * GForceFactor

    local_load_factor = (
        1.0 +
        vertical_acceleration_normalized /
        Gravity) if Gravity > 0 else 0.0
    local_g_force = (
        Gravity +
        vertical_acceleration_normalized) if Gravity > 0 else 0.0
...
func request_energy(energy_type: String, amount: float) -> bool:
    if (energy_type not in energy_budget_frame) or (
            energy_type not in available_energy):
        return false
...
func load_energy(energy_type: String, value: float) -> bool:
    ...
    if energy_type not in energy_containers_by_type:
        return false
    ...
    for this_container in selected_energy_containers:
        if this_container.ContainerActive:
            if not this_container.load_energy(
                    amount_per_container):  # returns true when full
